...
public class ExpectPaymentMethodValidationRule
  extends InputValidator<CustomPayment> {
  ...
  private final List<String> validPaymentMethods = List.of(
    ...
    PaymentMethodConstants.PAYMENT_METHOD_DIRECT_DEBIT
  );
  ...
  public List<ExtensionError> validate(CustomPayment payment) {
    ...
    if (errors.isEmpty() && !validPaymentMethods.contains(paymentMethod)) {
      errors.add(
        invalidInputError(
          String.format("Unrecognized payment method: %s", paymentMethod)
        )
      );
    }
