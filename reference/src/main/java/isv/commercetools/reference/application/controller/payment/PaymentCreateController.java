...
public class PaymentCreateController extends AbstractPaymentController {
  ...
  public PaymentCreateController(
    ...
    super(
      ...
      new PayloadLogger(
        objectMapper,
        LoggerFactory.getLogger(PaymentCreateController.class)
      ),
  ...
  @Override
  @RequestMapping(
    value = "/extension/payment/create",
    method = RequestMethod.POST
  )
  public ResponseEntity<ExtensionOutput> handle(
    @RequestBody final ExtensionInput<Payment> input
  ) {
    payloadLogger.log("Received payment create", input);
    final CustomPayment payment = new CustomPayment(
      input.getResource().getObj()
    );
    var extensionOutputResponseEntity = validateAndProcess(payment);
    payloadLogger.log(
      "Returning response",
      extensionOutputResponseEntity.getBody()
    );
