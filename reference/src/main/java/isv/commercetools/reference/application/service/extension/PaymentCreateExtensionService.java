...
public class PaymentCreateExtensionService implements ExtensionService {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(
    PaymentCreateExtensionService.class
  );
  ...
  public PaymentCreateExtensionService(
    @Qualifier(
      "paymentCreateServiceMap"
    ) Map<String, PaymentService> paymentCreateServiceMap
  ) {
  ...
  public ExtensionOutput process(CustomPayment payment) {
    ...
    try {
      ...
      LOGGER.error(
        "Failed to check if card is enrolled in payer authentication",
        e
      );
      extensionOutput =
        new ExtensionOutput()
  ...
  private PaymentService getCreateHandlerServiceForPayment(
